...
To integrate DyResearch into Obsidian, download the latest release directly from Obsidian Community Plugins or directly from its [Github page](https://github.com/dyresearch-obsidian).  

Once the plugin is enabled, you can start the backend app. 

---

## 🚀 Starting the BackEnd
The DyResearch app can be run locally or via docker compose.
...
1. Option one is to use the Obsidian's settings UI: 
   - Restart Obsidian.
...
2. Second option is to ensure a `config.env` file is correctly populated in the root directory of this project with your API keys, the session storage URI and the obsidian vault path. 

See the [configuration example file](config.env.example) for variables names.  

   ```env

   OBSIDIAN_VAULT_PATH=/Users/yourusername/Desktop/your-vault

   SESSION_SERVICE_URI=postgresql+asyncpg://adk_user:adk_password@postgres:5432/adk_history
